...
class SymMisspecShiftDataGenerator(SmallXShiftDataGenerator):
    def _get_prob(self, X, A):
        interaction = (A[:, np.newaxis] - 0.5) * 2 * X
        a_x_xa = np.concatenate([A[:, np.newaxis], X, interaction], axis=1)
        beta = self.source_beta if not self.is_shifted else self.target_beta
...
class GradualShiftAllDataGenerator(SmallXShiftDataGenerator):
    shift_speed = 0.05
    def _get_prob(self, X, A):
        interaction = (A[:, np.newaxis] - 0.5) * 2 * X
        a_x_xa = np.concatenate([A[:, np.newaxis], X, interaction], axis=1)
        ...
        if self.is_shifted:
            ...
            shift_sign = (prob > 0.5) * -1 + (prob < 0.5)
            delta_prob = (
                shift_sign * curr_prob_shift * (A[:, np.newaxis] == self.shift_A)
            )
            prob = to_safe_prob(prob + delta_prob, eps=0)
        return prob
    
class GradualSymMisspecShiftDataGenerator(SmallXShiftDataGenerator):
    shift_speed = 0.05
    def _get_prob(self, X, A):
        interaction = (A[:, np.newaxis] - 0.5) * 2 * X
        a_x_xa = np.concatenate([A[:, np.newaxis], X, interaction], axis=1)
